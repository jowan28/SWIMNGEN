...
def generator_output_layer(res):
    ...
    output_layer.add(layers.Conv2D(4,(1,1),input_shape=(res,res,128),activation = tf.nn.tanh))
...
def final_generator_block(output_resolution):
    ...
    block.add(layers.LeakyReLU())
    block.add(layers.Lambda(lambda x: tf.image.resize(x, [output_resolution, output_resolution])))
    block.add(layers.Conv2DTranspose(128,(4,4),padding='same'))
...
def final_discriminator_block(input_resolution):
    ...
    block = tf.keras.Sequential()
    block.add(layers.Conv2D(64,(3,3),padding='same',input_shape=(input_resolution,input_resolution,64)))
    block.add(layers.BatchNormalization())
    block.add(layers.LeakyReLU())
    block.add(layers.Dropout(0.4))
    block.add(layers.Lambda(lambda x: tf.image.resize(x, [output_resolution, output_resolution])))
...
class conditional_generator(tf.keras.Model):
    def __init__(self,resolution,model='not yet defined',classes=10,latent_shape=100):
    ...
    def grow(self,trainable_var):
        ...
        convlayer = layers.Conv2D(4,(1,1),input_shape=(res,res,128),activation=tf.nn.tanh)
    ...
    def prune(self,trainable_var):
        ...
        self.model = tf.keras.Model([latent_input,label_input],fully_grown,name='grown')
    ...
    def final_growth(self,output_resolution,trainable_var):
        ...
        dying_branch.add(convlayer)
        dying_branch.add(layers.Lambda(lambda x: 0.5*x + 0.5))
        dying_branch.add(layers.Lambda(lambda x: tf.image.resize(x, [output_resolution, output_resolution])))
        dying_branch._name = 'dying_branch'
...
class conditional_discriminator(tf.keras.Model):
    def __init__(self,resolution,model='not defined yet',classes=10,embedding_dim=100):
    ...
    def grow(self,trainable_var):
        ...
        full_encoder = conditional_discriminator_input_layer(2*resolution,self.classes)
    ...
    def final_growth(self,input_resolution,trainable_var):
        ...
        #the input
        label_input = tf.keras.Input(shape=(self.classes,))
        full_image_input = tf.keras.Input(shape=(input_resolution,input_resolution,4,))
        full_encoder = conditional_discriminator_input_layer(input_resolution,self.classes)
        full_encoder._name = 'input_encoder'
        half_image_input = layers.Lambda(lambda x: tf.image.resize(x, [intermediate_resolution, intermediate_resolution]))(full_image_input)
        half_encoder = self.model.layers[2]
        half_encoder._name = 'dying_encoder'
    ...
    def prune(self,trainable_var):
        ...
        label_input = tf.keras.Input(shape=(self.classes,))
        image_input = tf.keras.Input(shape=(self.resolution,self.resolution,4,))
        if isinstance(self.model.layers[2],keras.engine.functional.Functional) and isinstance(self.model.layers[4],keras.engine.sequential.Sequential):
        ...
        self.model = tf.keras.Model([image_input,label_input],fully_grown,name='grown')
...
class dataset:
    ...
    def get_data(self,resolution,BATCH_SIZE):
        ...
        def scale_dataset(target_shape,path='/home2/vcqf59/Large_aug_swimseg/'):
            ...
            return Z
        save_file = '/home2/vcqf59/Classified/masters-project/dataset pickle/half_swimnseg_'+str(resolution)+'_pickled_dataset'
        #check to see if folder exists
        if os.path.isdir(save_file)==False:
            swimseg = scale_dataset((resolution,resolution,3))
            swimseg_label = [1 for i in swimseg]
            swinseg = scale_dataset((resolution,resolution,3),path='/home2/vcqf59/Large_aug_swinseg/')
            ...
            #else: 
            #self.batches = 
            BUFFER_SIZE = int(np.max(np.shape(swimseg)))+int(np.max(np.shape(swinseg)))
...
class ConditionalGAN(keras.Model):
    def __init__(self, resolution, latent_dim, classes=2, final_resolution='not defined',csv_log='not_defined',update_threshold=0,noise_factor=0,transfer=False):
        super().__init__()
        self.resolution = resolution
        self.discriminator = conditional_discriminator(resolution,classes=classes,embedding_dim=latent_dim)
        self.generator = conditional_generator(resolution,classes=classes,latent_shape=latent_dim)
    ...
    def add_gaussian_noise(self,image):
        ...
        with tf.name_scope('Add_gaussian_noise'):
            noise = tf.random.normal(shape=tf.shape(image), mean=0.0, stddev=(self.noise_factor)/(255), dtype=tf.float32)
    ...
    def train_step(self,images,labels):
        ...
        with tf.GradientTape() as disc_tape:
            generated_images = self.generator.model([noise, one_hot_labels], training=True)
            
            real_output = self.discriminator.model([self.add_gaussian_noise(images), one_hot_labels], training=True)
            fake_output = self.discriminator.model([self.add_gaussian_noise(generated_images), one_hot_labels], training=True)
    ...
    def reg_train(self,data):
        for epoch in range(self.train_steps):
            ...
            if epoch%100 == 0:
                self.generator.model.save('/home2/vcqf59/Classified/masters-project/self_attention/GAN_models/non_convex_last_layer_final_self_attention_gan_g_model_{resolution}'.format(resolution=self.resolution))
                self.discriminator.model.save('/home2/vcqf59/Classified/masters-project/self_attention/GAN_models/non_convex_last_layer_final_self_attention_gan_d_model_{resolution}'.format(resolution=self.resolution))
            self.write_to_log(epoch,self.disc_loss_tracker.result().numpy(),self.gen_loss_tracker.result().numpy())
    ...
    def generate_and_save_images(self):
      ...
      # This is so all layers run in inference mode (batchnorm).
        ...
        one_hot_test_labels = keras.utils.to_categorical(test_labels,num_classes=self.classes)
        predictions = self.generator.model([test_input,one_hot_test_labels], training=False)
...
gan = ConditionalGAN(initial_resolution,128,final_resolution=75,csv_log=save_file,update_threshold=threshold,classes=2,noise_factor=noise,transfer=False)
...
resolution_increase = int(np.log2(gan.final_resolution/gan.resolution))  #how many times the resolution will double
#batch set
gan.batch_set()
#epoch set
gan.epoch_set()
